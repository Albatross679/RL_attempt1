...
# ============================================================
# SAC Training Script for Snake Approach Task
# ============================================================
...
SCRIPT_PATH="$SCRIPT_DIR/train_snake_approach_sac.sh"
...
#     bash train_snake_approach_sac.sh
#
#   With custom parameters:
#     NUM_ITERATIONS=10000 bash train_snake_approach_sac.sh
#
#   Run in background with logging:
#     nohup bash train_snake_approach_sac.sh > train_snake.log 2>&1 &
#
#   Monitor training with TensorBoard (in separate terminal):
#     tensorboard --logdir $PROJECT_ROOT/results --port 6006

# -------------------- Configurable Parameters --------------------
export ROOT_DIR="${ROOT_DIR:-${PROJECT_ROOT}/results/snake_approach_sac_$(date +%Y%m%d_%H%M%S)}"
export NUM_PARALLEL_ENVS="${NUM_PARALLEL_ENVS:-200}"
...
echo ""
echo "Starting Snake Approach training. Monitor with:"
echo "  tensorboard --logdir $ROOT_DIR --port 6006"
echo "Then open: http://localhost:6006"
echo ""

python3 -m alf.bin.train \
    --conf confs/snake_approach_conf.py \
    --root_dir "$ROOT_DIR" \
