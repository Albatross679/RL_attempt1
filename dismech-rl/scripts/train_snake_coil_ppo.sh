...
# ============================================================
# PPO Training Script for Snake Coil Task
# ============================================================
...
SCRIPT_PATH="$SCRIPT_DIR/train_snake_coil_ppo.sh"
...
#     bash train_snake_coil_ppo.sh
#
#   With custom parameters:
#     NUM_ITERATIONS=10000 bash train_snake_coil_ppo.sh
#
#   Run in background with logging:
#     nohup bash train_snake_coil_ppo.sh > train_snake_coil_ppo.log 2>&1 &
#
#   Monitor training with TensorBoard (in separate terminal):
#     tensorboard --logdir $PROJECT_ROOT/results --port 6006

# -------------------- Configurable Parameters --------------------
export ROOT_DIR="${ROOT_DIR:-${PROJECT_ROOT}/results/snake_coil_ppo_$(date +%Y%m%d_%H%M%S)}"
...
echo ""
echo "Starting Snake Coil PPO training. Monitor with:"
echo "  tensorboard --logdir $ROOT_DIR --port 6006"
echo "Then open: http://localhost:6006"
echo ""

python3 -m alf.bin.train \
    --conf confs/snake_coil_ppo_conf.py \
    --root_dir "$ROOT_DIR" \
