...
class BaseEnv(AlfEnvironment, ABC):
    ...
    def _step(self, action: np.ndarray) -> TimeStep:
        ...
        # Store previous observation for PBRS
        ...
        self._curr_obs = obs  # Store current observation for PBRS
    ...
    def _check_success(self, obs: NamedTuple) -> bool:
        """
        Check if the task was completed successfully.

        Override in child classes to enable early termination on success.
    ...
    def _reset(self) -> TimeStep:
        ...
        # Reset PBRS observation tracking
        self._prev_obs = None
        self._is_terminal = False

        obs = self._generate_observation()
        self._curr_obs = obs  # Initialize current observation for PBRS
