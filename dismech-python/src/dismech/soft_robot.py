...
class SoftRobot:
    def __init__(self, geom: GeomParams, material: Material,
        ...
        #     np.concatenateenate((geo.rod_edges, geo.rod_shell_joint_edges)
    ...
    def _init_springs(self, geo: Geometry):
        ...
        nodes_ind = np.concatenate([geo.rod_stretch_springs,
                               geo.shell_stretch_springs], axis=0)
        ref_len = self.__ref_len[:n_rod + n_shell]
        EA = np.concatenate([np.full(n_rod, self.__EA),
